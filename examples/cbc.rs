...
fn main() {
    ...
    // Encrypt data
    let mut cipher = match Cipher::init(Operation::Encrypt, key, Mode::Cbc(iv), Padding::PKCS7) {
        Ok(c) => c,
        Err(err) => panic!("{:?}", err),
    };
    ...
    // Decrypt data
    let mut cipher = match Cipher::init(Operation::Decrypt, key, Mode::Cbc(iv), Padding::PKCS7) {
        Ok(c) => c,
        Err(err) => panic!("{:?}", err),
    };
